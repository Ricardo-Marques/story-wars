...
const ScoreRow = styled.div<{ highlighted?: boolean }>`
  ...
  background: ${(p) => (p.highlighted ? `${theme.colors.primary}22` : theme.colors.bgCard)};
  border-radius: ${theme.radii.sm};
...
export const PlayPage = observer(function PlayPage() {
  ...
  const { phase, playState, isLeader, isHost, players, currentStory, config, myPlayerId, storiesForCurrentTopic, votesForCurrentStory } = gameStore;

  const [muted, setMuted] = useState(() => !isTtsEnabled());
  const [typewriterDone, setTypewriterDone] = useState(false);
  // TTS boundary-driven reveal position — owned here so it survives sub-phase changes
  ...
  // TTS — starts when a new story appears in READING, keeps playing into VOTING
  // Reads the topic text first, then the story
  useEffect(() => {
    if (playState.subPhase !== 'READING' || !currentStory) return;

    const topicId = config.topicIds[playState.currentTopicIndex];
    const topic = TOPICS.find((t) => t.id === topicId);
    const topicPrefix = topic ? `Topic ${playState.currentTopicIndex + 1}: ${topic.text}. ` : '';
    const prefixLen = topicPrefix.length;

    speak(topicPrefix + currentStory.text, {
      onBoundary: (charIndex, charLength) => {
        // Only reveal story text (after the topic prefix)
        if (charIndex + charLength > prefixLen) {
          setRevealUpTo(charIndex + charLength - prefixLen);
        }
      },
      onEnd: () => {
        if (currentStory) {
  ...
  const nobodyGuessedRight =
    playState.subPhase === 'REVEAL' && currentStory
      ? !votesForCurrentStory.some(
          (v) => v.voterId !== currentStory.authorId && v.targetPlayerId === currentStory.authorId,
        )
      : false;
    ...
    <PageWrap>
      <TopBar>
        <TopicProgress>
          ...
          {' — '}Story {playState.currentStoryIndex + 1}/{storiesForCurrentTopic.length}
        </TopicProgress>
      ...
      {playState.subPhase === 'VOTING' && (
        <>
          <Timer
            seconds={playState.voteTimerSecondsLeft > 0 ? playState.voteTimerSecondsLeft : config.voteTimerSeconds}
            running={!timerPaused}
          />
          <VotePanel
            players={players}
              ...
              .map((p) => {
                ...
                return (
                  <ScoreRow key={p.id} highlighted={delta > 0}>
                    <ScoreAvatar src={getAvatarDataUri(p.avatarSeed)} alt={p.name} />
